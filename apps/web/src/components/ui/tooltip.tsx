  ...
  return (
    <TooltipPrimitive.Portal>
      <TooltipPrimitive.Content
        ...
        className={cn(
          'border-border-strong bg-surface-floating text-popover-foreground animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 z-[80] w-fit max-w-64 origin-(--radix-tooltip-content-transform-origin) rounded-xl border px-3 py-1.5 text-xs text-balance shadow-[var(--shadow-panel)]',
          className,
        )}
        {...props}
      >
        {children}
        <TooltipPrimitive.Arrow className="bg-surface-floating fill-surface-floating border-border-strong z-[80] size-2.5 translate-y-[calc(-50%_-_2px)] rotate-45 rounded-[2px] border-r border-b" />
      </TooltipPrimitive.Content>
    </TooltipPrimitive.Portal>
  );
}
