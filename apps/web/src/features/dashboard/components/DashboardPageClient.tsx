...
export default function DashboardPageClient({
  initialStats,
}: DashboardPageClientProps): React.ReactNode {
  ...
  const [dashboardSnapshot, setDashboardSnapshot] =
    useState<DashboardSnapshot | null>(() =>
      initialStats
        ? { scope: dashboardAccessScope, stats: initialStats }
        : null,
    );
  ...
  useEffect((): void => {
    if (dashboardSnapshot?.scope === dashboardAccessScope) return;
    void fetchDashboardStats();
  }, [dashboardAccessScope, dashboardSnapshot?.scope, fetchDashboardStats]);
