...
type RouteContext = { params: Promise<{ emailId: string; id: string }> };
...
export async function PATCH(
  ...
  const parsed = updatePersonEmailSchema.safeParse(body.data);
  if (!parsed.success) {
  ...
  const { emailId, id } = await context.params;

  try {
    await assertPersonFeatureReady();

    return withPrivateNoStore(
      apiSuccess(await updatePersonEmail(id, emailId, parsed.data, auth.user)),
    );
  } catch (error) {
    return handlePersonApiError('PERSON_EMAIL_UPDATE', error, request);
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return withPrivateNoStore(
      apiSuccess({
        person: await deletePersonEmail(id, emailId, parsed.data, auth.user),
      }),
    );
  } catch (error) {
    return handlePersonApiError('PERSON_EMAIL_DELETE', error, request);
  }
}
