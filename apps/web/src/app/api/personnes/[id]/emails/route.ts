...
import { createPersonEmailSchema } from '$features/persons/schemas/person.schemas';
import { addPersonEmail } from '$features/persons/server/person.service';
import {
...
export async function POST(
  ...
  const parsed = createPersonEmailSchema.safeParse(body.data);
  if (!parsed.success) {
  ...
  try {
    ...
    return withPrivateNoStore(
      apiSuccess(await addPersonEmail(id, parsed.data, auth.user), 201),
    );
  } catch (error) {
    return handlePersonApiError('PERSON_EMAIL_CREATE', error, request);
  }
}
