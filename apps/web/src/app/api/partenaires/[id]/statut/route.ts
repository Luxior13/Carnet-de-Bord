...
import { updatePartnerStatusSchema } from '$features/partners/schemas/partner.schemas';
import { updatePartnerStatus } from '$features/partners/server/partner.service';
import {
...
export async function PATCH(
  ...
  const parsed = updatePartnerStatusSchema.safeParse(body.data);
  if (!parsed.success) {
  ...
  try {
    ...
    return withPartnerNoStore(
      apiSuccess({
        partner: await updatePartnerStatus(
          id,
          parsed.data,
          auth.user,
          canViewPersons,
    ...
    return handlePartnerApiError('PARTNER_STATUS_UPDATE', error, request);
  }
}
