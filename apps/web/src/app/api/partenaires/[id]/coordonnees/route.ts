...
import { addPartnerChannel } from '$features/partners/server/partner.service';
import {
...
export async function POST(
  ...
  const parsed = createPartnerChannelSchema.safeParse(body.data);
  if (!parsed.success) {
  ...
  try {
    ...
    return withPartnerNoStore(
      apiSuccess({
        partner: await addPartnerChannel(
          id,
          parsed.data,
          auth.user,
          canViewPersons,
    ...
    return handlePartnerApiError('PARTNER_CHANNEL_CREATE', error, request);
  }
}
